...
fn main() {
    ...
    if !result.hands.is_empty() {
        ...
        for (i, hand) in result.hands.iter().take(5).enumerate() {
            ...
            let hero_participated = hand.players.iter()
                .any(|p| p.name == "thesmoy");
        ...
        let total_actions: usize = result.hands.iter()
            .map(|h| h.actions.len())
            .sum();
        
        let hands_with_hero: usize = result.hands.iter()
            .filter(|h| h.players.iter().any(|p| p.name == "thesmoy"))
            .count();
        
        let total_pot: i64 = result.hands.iter()
            .map(|h| h.pot.total_cents)
            .sum();
        
        let total_rake: i64 = result.hands.iter()
            .map(|h| h.pot.rake_cents)
            .sum();
